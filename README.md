...
这不是新的生图模型，而是一个“分工、排队、监控和归档”工具：一次要生成 4 张或更多图片时，它把任务分给本地 Codex 和 ChatGPT 网页端并行处理。本地 Codex 与网页端是两条独立启动链路，网页操作不会阻塞已经确认的本地生成。

它不需要 OpenAI API Key，也不会替你绕过登录、验证码或发送确认。

网页端最后的 **发送** 仍由你在每个 ChatGPT 页面手动点击。发送后，扩展会自动监控生成结果、下载图片并保存到本批次的 `图片/` 文件夹；当某个通道空出位置时，它会准备下一页，但新网页任务仍需要你点击发送。本地 Codex 应在确认后立即开始，不需要等待网页端。
...
```text
已确认任务分配方案，请立即创建批次并开始本地 Codex 生成。
本地 Codex 不要等待 Chrome 登录、网页预检、创建网页任务页或网页端发送。
暂时不要发送任何网页任务。
批次根目录使用：/绝对路径/批次存放目录
...
如果你只回复“已确认分配方案”，Skill 必须继续询问：

> 分配方案已确认。是否现在启动本地 Codex 生成任务？回复“开始本地生成”即可。

回复“开始”“开始本地生成”或“确认开始本地 Codex 生成”后，本地 Codex 任务应立即创建并开始。不能把“已分配”或“已创建批次”当成“已启动”。
...
任务被分配到 Codex，只表示它进入 Codex 通道；能否同时启动多个隔离的 Codex 生图任务，取决于当前 Codex Desktop 环境的能力。若环境探针提示不可用，不能把“已分配”当成“已生成”，网页端任务仍可独立运行。
...
4. 用户确认分配方案后，Skill 单独询问是否“开始本地 Codex 生成”。用户明确同意后，Skill 创建新的 `图片批次_<batch_id>`，写入 `manifest.json` 与事件日志，并立即创建全部已分配的本地 Codex 任务；这个动作不等待 Chrome 或网页端。
5. 在扩展设置页粘贴命令输出的 `extensionLoadPath`（当前批次目录）并选择“载入批次（不发送）”。页面会显示每张任务、执行通道、提示词状态和所有附件，不展开长提示词正文。
